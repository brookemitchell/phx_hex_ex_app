defmodule PhxAppWeb.CartItemController do
  use PhxAppWeb, :controller

  alias PhxApp.{ShoppingCart, Catalog}
  ...
  def create(conn, %{"product_id" => product_id}) do
    ...
    case ShoppingCart.add_item_to_cart(conn.assigns.cart, product) do
      ...
      {:error, _} ->
        conn
        |> put_flash(:error, "There was an error adding item to your cart")
        |> redirect(to: Routes.cart_path(conn, :show))
    end
  end

  def delete(conn, %{"id" => product_id}) do
    {:ok, _} = ShoppingCart.remove_item_from_cart(conn.assigns.cart, product_id)
